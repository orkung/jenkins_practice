...
ENV SUMMARY="Redis in-memory data structure store, used as database, cache and message broker"              \
    DESCRIPTION="Redis $REDIS_VERSION available as container, is an advanced                                \
    key-value store.  It is often referred to as a data structure server since                              \
    keys can contain strings, hashes, lists, sets and sorted sets. You can run                              \
    atomic operations on these types, like appending to a string; incrementing                              \
    the value in a hash; pushing to a list; computing set intersection, union                               \
    and difference; or getting the member with highest ranking in a sorted set.                             \
...
LABEL summary="$SUMMARY"                                                                                    \
      ...
      version="1"                                                                                           \
      com.redhat.licenseterms="https://www.redhat.com/en/about/red-hat-end-user-license-agreements#rhel"    \
